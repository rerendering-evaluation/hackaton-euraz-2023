...
export namespace MockApi {
  ...
  function getDataMarkState(mark: MarkType) {
    if (isPresent(mark.maxDanger) && mark.value >= mark.maxDanger || isPresent(mark.minDanger) && mark.value <= mark.minDanger) return 'danger';
    if (isPresent(mark.maxCaution) && mark.value >= mark.maxCaution || isPresent(mark.minCaution) && mark.value <= mark.minCaution) return 'caution';
    return 'ok';
  }
  const valves: ValveElementType[] = [{
    ...
    name: 'Клапан',
    get isOpened() {
      return this.data.position.value === 100;
    },
    get isClosed() {
      return this.data.position.value === 0;
    },
    data: {
      position: {
        id: 'position',
    ...
    name: 'Клапан',
    get isOpened() {
      return Math.round(this.data.position.value) === 100;
    },
    get isClosed() {
      return Math.round(this.data.position.value) === 0;
    },
    data: {
      position: {
        id: 'position',
  ...
  function getVibraition(bearer: BearerElementType) {
    if (bearer.data.verticalVibration?.state === 'danger' || bearer.data.horizontalVibration?.state === 'danger' || bearer.data.axialVibration?.state === 'danger') return 'danger';else if (bearer.data.verticalVibration?.state === 'caution' || bearer.data.horizontalVibration?.state === 'caution' || bearer.data.axialVibration?.state === 'caution') return 'caution';else if (bearer.data.verticalVibration?.state === 'ok' && bearer.data.horizontalVibration?.state === 'ok' && bearer.data.axialVibration?.state === 'ok') return 'ok';else return undefined;
  }
  function getBearerState(bearer: BearerElementType) {
    if (bearer.temperature === 'danger' || bearer.vibration === 'danger') return 'danger';else if (bearer.temperature === 'caution' || bearer.vibration === 'caution') return 'caution';else return 'ok';
  }
  ...
  const bearers: BearerElementType[] = [{
    ...
    location: 'bearer-1',
    get temperature() {
      return getTemeperature(this);
    },
    get vibration() {
      return getVibraition(this);
    },
    get state() {
      return getBearerState(this);
    },
    data: {
      temperature: {
        id: 'temperature',
  ...
  const rotors: RotorElementType[] = [{
    ...
    get replacementWere() {
      return DateTime.from_dd_MM_yyyy(this.replacementDate)!.getDays();
    },
    // через сколько дней заменить (прогноз замены)
    replacementForecast: 12,
    // текущее состояние ротора, зависит от прогноза замены
    get replacementForecastState() {
      return this.replacementForecast <= 3 ? 'danger' : this.replacementForecast <= 10 ? 'caution' : 'ok';
    }
  }, {
    id: nextId(),
    type: 'rotor',
  ...
  export const exhausters = [{
    ...
    valve: valves[0],
    get allElements(): ElementType[] {
      return [...this.bearers, this.oil, this.rotor, this.cooler, this.mainEngine, this.gasCollector, this.valve];
    }
  }, {
    id: nextId(),
  ...
  export const sinteringMachines = [{
    id: nextId(),
    name: 'Агломашина №1',
    exhausters: exhausters.slice(0, 2)
  }, {
    id: nextId(),
    name: 'Агломашина №2',
    exhausters: exhausters.slice(2, 4)
  }];
