export class DateTime {
  constructor(public year: number, public month: number = 0, public day: number = 0, public hour: number = 0, public minute: number = 0, public second: number = 0) {}
  ...
  public static from_yyyy_MM_dd_hh_mm(date?: string) {
    ...
    if (match) return new DateTime(+match.groups!.year!, +match.groups!.month!, +match.groups!.day!, +match.groups!.hour!, +match.groups!.minute!);
  }
  ...
  to_yyyy_MM_dd_HH_mm_ss() {
    return `${(this.year + '').padStart(4, '0')}-${(this.month + '').padStart(2, '0')}-${(this.day + '').padStart(2, '0')}T` + `${(this.hour + '').padStart(2, '0')}:${(this.minute + '').padStart(2, '0')}:${(this.second + '').padStart(2, '0')}`;
  }
  to_yyyy_MM_dd_HH_mm() {
    return `${(this.year + '').padStart(4, '0')}-${(this.month + '').padStart(2, '0')}-${(this.day + '').padStart(2, '0')}T` + `${(this.hour + '').padStart(2, '0')}:${(this.minute + '').padStart(2, '0')}`;
  }
  ...
  // to 01-01-23
  to_dd_MM_yy(divider = '-') {
    return `${(this.day + '').padStart(2, '0')}${divider}` + `${(this.month + '').padStart(2, '0')}${divider}` + `${(this.year % 100 + '').padStart(2, '0')}`;
  }
