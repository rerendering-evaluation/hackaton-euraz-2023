...
export namespace BearerElementCard {
  ...
  export function BearerElementCard({
    bearer,
    style
    ...
    return <Card style={style}>
      ...
      <MarksCol>
        ...
        <MarkRow state={bearer.data.temperature.state}>
          <div style={{
            flex: 1
          }}><div>{getMarkName(bearer.data.temperature.id)}</div></div>
        ...
        {bearer.data.verticalVibration && <MarkRow state={bearer.data.verticalVibration.state}>
          <div style={{
            flex: 1
          }}><div>{getMarkName(bearer.data.verticalVibration.id)}</div></div>
        ...
        {bearer.data.horizontalVibration && <MarkRow state={bearer.data.horizontalVibration.state}>
          <div style={{
            flex: 1
          }}><div>{getMarkName(bearer.data.horizontalVibration.id)}</div></div>
        ...
        {bearer.data.axialVibration && <MarkRow state={bearer.data.axialVibration.state}>
          <div style={{
            flex: 1
          }}><div>{getMarkName(bearer.data.axialVibration.id)}</div></div>
  ...
  function getMarkName(markId: MarkType['id']) {
    switch (markId) {
      case "temperature":
        return 'Т, °C';
      case "vertical-vibration":
        return 'В, мм/с';
      case "horizontal-vibration":
        return 'Г, мм/с';
      case "axial-vibration":
        return 'О, мм/с';
      default:
        return 'null';
    }
