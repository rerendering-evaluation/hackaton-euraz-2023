...
export namespace Item {
  ...
  export function Item(props: ItemProps) {
    ...
    onMouseLeave={() => props.setHovered(it, false, 'leave')}>
      ...
      {it.type === 'bearer' && <>
          <Bearer />
          <WrapFlex1><ItemTitle>{it.name}</ItemTitle></WrapFlex1>
        {it.temperature && <MarkView.MarkView mark={{
          type: 'temperature',
          state: it.temperature
        }} />}
        {it.vibration && <MarkView.MarkView mark={{
          type: 'vibration',
          state: it.vibration
        }} />}
      </>}
  ...
  const Item_ = styled.div`
      ...
      align-items: center;
    `;
  const Oil = styled(WaterDropIc).attrs({
    mainColor: '#262626',
    size: '1em'
  })``;
  const Bearer = styled(BearerIc).attrs({
    mainColor: '#262626',
    size: '1em'
  })``;
